...
└── templates/
    ...
    └── system_intro.html
└── database/              # 数据库相关代码（保持不变）
│   ├── db_manager.py
│   ├── database_schema.db  # 数据库文件
│   ├── database_schema.sql
│   └── scheduler.py
```
...
- **1.打开终端，先退出conda baes 环境 conda deactivate**(自己)
