...
class CertificateValidityAnalyzer:
    ...
    def _analyze_certificate_details(self, cert: x509.Certificate, filepath: str, cert_hash: str) -> Dict:
        ...
        try:
            ...
            days_valid = (not_after - not_before).days if not_after > not_before else 0

            # 加密信息
            public_key = cert.public_key()
            key_algorithm = type(public_key).__name__          # RSAPublicKey, EllipticCurvePublicKey 等
            key_size = getattr(public_key, 'key_size', None)   # RSA 密钥长度；EC 没有 key_size 属性，返回 None
            signature_algorithm = cert.signature_algorithm_oid._name

            # 密码学强度描述（保留原有 crypto_strength 用于图表等）
            crypto_strength = self._analyze_crypto_strength(public_key)
