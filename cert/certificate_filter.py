    ...
    def _parse_handshake_messages(self, payload: bytes, tls_version: int) -> None:
        ...
        while hs_offset + 4 <= len(payload):
            try:
                ...
                if hs_type == 11:
                    # 检查TLS版本：1.3及以上版本由于加密，无法提取有效证书，直接跳过
                    if tls_version > 0x0303:  # TLS 1.3 (0x0304) 及以上
                        if self.debug:
                            print(f"跳过TLS 1.3 Certificate消息 (版本: 0x{tls_version:04x})，证书数据已加密无法提取")
                        self.stats['skipped_tls13_certs'] = self.stats.get('skipped_tls13_certs', 0) + 1
